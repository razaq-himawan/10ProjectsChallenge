...
import { geometry as _geometry } from '../utils/hereMaps.js';
  ...
  const places = [
    {
      ...
      image:
        'https://images.unsplash.com/photo-1707343848552-893e05dba6ac?q=80&w=1280&auto=format&fit=crop',
    },
    {
      title: 'Pantai Kuta',
      price: 0,
  ...
  try {
    const newPlace = await Promise.all(
      places.map(async (place) => {
        let geoData = await _geometry(place.location);
        if (!geoData) {
          geoData = { type: 'Point', coordinates: [116.32883, -8.90952] };
        }
        return {
          ...place,
          author: '66a49e9ab018bc0e81fe8be2',
          images: {
            url: 'public/images/image-1722852023653-355022022',
            filename: 'image-1722852023653-355022022.jpg',
          },
          geometry: { ...geoData },
        };
      })
